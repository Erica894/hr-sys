<template>
  <div class="hr-page">
    <PageHeader
      title="年终奖预算池"
      subtitle="按员工类别配置公司层年终奖预算，并下发到部门 / 中心"
    />
    ...
    <el-table
      ...
      v-loading="loading"
    >
      <el-table-column label="员工类别" prop="label" width="160" fixed />
      <el-table-column label="预算 / 已分配 / 剩余">
        <template #default="{ row }">
          <BudgetCell
            :cell="row.cell"
            :disabled="loading || saving"
            @update:budget="(v: number) => (row.cell.budget_amount_cny = v)"
            @distribute="() => openDistribute(row.cell.employee_category_1, row.cell)"
          />
        </template>
      </el-table-column>
    </el-table>
    ...
    >
      ...
      <el-table-column label="预算（CNY）" width="220" align="right">
        <template #default="{ row }">
          <el-input-number
      ...
      <template #empty>
        <EmptyHint
          title="暂未下发"
          description="尚未将公司层年终奖预算切到部门 / 中心，请使用上方下发按钮"
          icon="box"
        />
      </template>
    </el-table>
...
type Row = { label: string; cell: Cell }
const tableRows = ref<Row[]>([])

function rebuildRows() {
  const map: Record<string, Cell> = {}
  for (const r of rows.value) map[r.employee_category_1] = r
  const mk = (cat: string): Cell =>
    map[cat] || {
  ...
  tableRows.value = [
    { label: "管理干部", cell: mk("MANAGEMENT") },
    { label: "员工", cell: mk("STAFF") },
  ]
}
...
async function load() {
  ...
  loading.value = true
  try {
    const r = await api.get(`/admin/reward-cycles/${cycleId.value}/bonus-budget/`)
...
async function saveTargets() {
  ...
  try {
    ...
    await api.patch(
      `/admin/reward-cycles/${cycleId.value}/bonus-budget/targets/`,
      { targets: items },
    )
  ...
  try {
    const payload = {
      rows: tableRows.value.map((r) => ({
        employee_category_1: r.cell.employee_category_1,
        budget_amount_cny: r.cell.budget_amount_cny,
      })),
    }
    const r = await api.put(`/admin/reward-cycles/${cycleId.value}/bonus-budget/`, payload)
    const cascade = r.data?.cascade || []
    if (cascade.length) {
      ElMessage.success(`预算已保存，并按已有规则联动更新 ${cascade.length} 类部门预算`)
    } else {
    ...
    if (detail?.error === "DEPT_REDUCE_BELOW_ALLOCATED") {
      const d = detail.detail || {}
      ElMessageBox.alert(
        `${d.employee_category_1 === "MANAGEMENT" ? "管理干部" : "员工"} 联动失败：` +
...
async function openDistribute(cat: "MANAGEMENT" | "STAFF", cell: Cell) {
...
async function distributeDryRun() {
  ...
  try {
    const r = await api.post(
      `/admin/reward-cycles/${cycleId.value}/bonus-budget/distribute/`,
      buildPayload(true),
    )
